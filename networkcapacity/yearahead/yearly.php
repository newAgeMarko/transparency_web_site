...
<body> 
	...
	function GetData(y_from) {  
		...
		$.getJSON("get_results.php", {year: y_from}, function (result) { 
			...
			if (result == null) {
				...
				var theaderHTML = '<tr class="subtitle bold"><td class="hour"><?= $_DIRECTION ?></td><td><?= $_TTC ?></td><td><?= $_TRM ?></td><td><?= $_NTC ?></td><td><?= $_ATC ?></td><td><?= $_AUCTION_PERIOD_FROM ?></td><td><?= $_AUCTION_PERIOD_TILL ?></td></tr>';
				...
				for (var i in result) {		
					...
					tbodyHTML += '<tr><td class="hour">' + result[i].granica_smer + '</td><td class="hour">' + result[i].TTC + '</td><td class="hour">' + result[i].TRM + '</td><td class="hour">' + result[i].NTC + '</td><td class="hour">' + result[i].ATC + '</td><td class="hour">' + result[i].period_od + '</td><td class="hour">' + result[i].period_do + '</td></tr>';
				}
	...
	//	var month = $('#datepicker-month').datepicker({dateFormat: 'MM yy'}).data('datepicker');
		...
		filename = "NETWORK_CAPACITY_YEARLY";
