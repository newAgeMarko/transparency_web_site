...
//	$data = array();
	...
	if ($format == "XML" && (($month != $month_to) || ($year != $year_to))) { 
		...
		$sql = "SELECT granica_smer, ATC, period_od, period_do, zahtevani_kapacitet, dodeljeni_kapacitet, ucesnici_podneli_zahtev, ucesnici_dobili_kapacitet, broj_ponuda, cena, zagusenje FROM resultsm WHERE period_od >= CONCAT(($year), '-$month-01') AND period_od <= CONCAT(($year_to),'-$month_to-30') ORDER BY year(period_od), month(period_od), field(granica_smer, 'HURS', 'RORS', 'ALRS', 'BARS', 'BGRS', 'HRRS', 'MERS', 'MKRS', 'RSHU', 'RSRO', 'RSAL', 'RSBA', 'RSBG', 'RSHR', 'RSME', 'RSMK')";
		$res = $conn->query($sql);
		...
		while ($year <= $year_to) {  
			if ($year==$year_to) {
				for ($j=$month; $j<=$month_to; $j++) { 
					$sql = "SELECT granica_smer, ATC, period_od, period_do, zahtevani_kapacitet, dodeljeni_kapacitet, ucesnici_podneli_zahtev, ucesnici_dobili_kapacitet, broj_ponuda, cena, zagusenje FROM resultsm WHERE month(period_od) = $j AND year(period_od) = $year ORDER BY year(period_od), month(period_od), field(granica_smer, 'HURS', 'RORS', 'ALRS', 'BARS', 'BGRS', 'HRRS', 'MERS', 'MKRS', 'RSHU', 'RSRO', 'RSAL', 'RSBA', 'RSBG', 'RSHR', 'RSME', 'RSMK')";	
					$res = $conn->query($sql);	
	...
	function MakeXML($border, $year, $month, $value) {
		...
		$xml_txt .= '<MonthlyAuctionResults-AllocatedCapacity>' . "\r\n";
		$xml_txt .= '	<Year v="' . $year . '"/>' . "\r\n";
		$xml_txt .= '	<Month v="' . $month . '"/>' . "\r\n";

		foreach ($value as $d) {
			$xml_txt .= '	<Direction>'  . "\r\n"; 
			$xml_txt .= '		<Direction v="' . $d['granica_smer'] . '"/>' . "\r\n";
			$xml_txt .= '		<MeasurementUnit v="MW"/>' . "\r\n";
			$xml_txt .= '		<ATC v="' . $d['ATC'] . '"/>' . "\r\n";
			$xml_txt .= '		<period_from v="' . $d['period_od'] . '"/>' . "\r\n";
			$xml_txt .= '		<period_to v="' . $d['period_do'] . '"/>' . "\r\n";
			$xml_txt .= '		<TotalRequestedCapacity v="' . $d['zahtevani_kapacitet'] . '"/>' . "\r\n";
			$xml_txt .= '		<TotalAllocatedCapacity v="' . $d['dodeljeni_kapacitet'] . '"/>' . "\r\n";
			$xml_txt .= '		<NumberOfAuctionParticipants v="' . $d['ucesnici_podneli_zahtev'] . '"/>' . "\r\n";
			$xml_txt .= '		<NumberOfAuctionParticipantsWithNonZeroAllocatedCapacity v="' . $d['ucesnici_dobili_kapacitet'] . '"/>' . "\r\n";
			$xml_txt .= '		<NumberOfBids v="' . $d['broj_ponuda'] . '"/>' . "\r\n";
			$xml_txt .= '		<MarginalPrice v="' . $d['cena'] . '"/>' . "\r\n";
			$xml_txt .= '		<Congestion v="' . $d['zagusenje'] . '"/>' . "\r\n";
			$xml_txt .= '	</Direction>' . "\r\n";
		}
